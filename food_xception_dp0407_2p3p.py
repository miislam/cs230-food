...
import time
filename = time.strftime("%Y%m%d_%H%M") + "_xception_dp0407_2p3p"
...
model.load_weights("20180607_1801_xception_dp0608_first.30-2.36.hdf5")
...
# serialize weights to HDF5
model.save_weights(filename + "_2p_weights.hdf5")
print("Saved model to disk")



nh = 4
print("Third pass 4 layers")
for layer in model.layers[:132-nh]:
    layer.trainable = False
for layer in model.layers[132-nh:]:
    layer.trainable = True
...
nh = 10
print("Sixth pass 10 layers")
for layer in model.layers[:132-nh]:
    layer.trainable = False
for layer in model.layers[132-nh:]:
    layer.trainable = True
model.compile(optimizer=Adam(lr=0.001, beta_1=0.9, beta_2=0.999), loss='categorical_crossentropy', metrics=['accuracy'])
checkpointer = ModelCheckpoint(filepath=filename + '_sixth.{epoch:02d}-{val_loss:.2f}.hdf5', verbose=1, save_best_only=False)
csv_logger = CSVLogger(filename + 'sixth.log')
model.fit_generator(generator,
                    validation_data=val_generator,
                    epochs=15,
                    verbose=1,
...
# serialize weights to HDF5
model.save_weights(filename + "_6p_weights.hdf5")
print("Saved model to disk")



nh = 12
print("Seventh pass 12 layers")
for layer in model.layers[:132-nh]:
    layer.trainable = False
for layer in model.layers[132-nh:]:
    layer.trainable = True
model.compile(optimizer=Adam(lr=0.001, beta_1=0.9, beta_2=0.999), loss='categorical_crossentropy', metrics=['accuracy'])
checkpointer = ModelCheckpoint(filepath=filename + '_seventh.{epoch:02d}-{val_loss:.2f}.hdf5', verbose=1, save_best_only=False)
csv_logger = CSVLogger(filename + 'seventh.log')
model.fit_generator(generator,
                    validation_data=val_generator,
                    epochs=15,
                    verbose=1,
...
# serialize weights to HDF5
model.save_weights(filename + "_7p_weights.hdf5")
print("Saved model to disk")