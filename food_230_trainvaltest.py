...
h = h5py.File('data/data_'+classes[0]+'.hdf5', 'r')
h.keys()
y_all = np.array(h.get('classes'), dtype='uint8') # Size (m, 101)
X_all = np.array(h.get('data'), dtype='uint8') # Size (m, n_h = 299 , n_w = 299, n_c = 3)
X_all = X_all[0:500,:,:,:]
y_all = y_all[0:500,:]
X_train, X_dev_test, y_train, y_dev_test = train_test_split(X_all, y_all, test_size=.4, stratify=y_all)
X_dev, X_test, y_dev, y_test = train_test_split(X_dev_test, y_dev_test, test_size=.25, stratify=y_dev_test)
h = h5py.File('all_data_300515.hdf5', 'w')
...
for fooditem in classes[1:]:
    ...
    X_dev, X_test, y_dev, y_test = train_test_split(X_dev_test, y_dev_test, test_size=.25, stratify=y_dev_test)
    h.close()
    with h5py.File('all_data_300515.hdf5', 'a') as hf:
